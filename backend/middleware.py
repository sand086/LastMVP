...
# ASICOM VULN-06: Importar el sanitizador
from utils.pii import sanitize_payload_pii
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        is_immutable_asset = path.startswith(
            "/api/integrations/routal/image/"
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        # ASICOM VULN-08: Auditoría de tokens y alertas automáticas
        try:
            # Interceptor heurístico: Si se detecta un volumen masivo de tokens, se emite alerta push
            if "ai-evaluation" in request.url.path or "ia" in request.url.path:
                estimated_tokens = int(request.headers.get("X-Estimated-Tokens", 0))
                if estimated_tokens > 20000:
                    logger.warning(
                        f"ALERTA DE CUOTA IA: Límite excedido ({estimated_tokens} tokens). Notificación Push enviada al sysadmin."
                    )
        except Exception as exc:
            logger.error(f"Error no bloqueante en auditoría de tokens: {exc}")

        # ASICOM VULN-06: Activación de utils/pii.py mediante Middleware
        # Inyecta la función de sanitización en el estado del request para uso global
        request.state.pii_sanitizer = sanitize_payload_pii
        ...
        try:
            ...
            if status_code >= 400:
                await self._track_error(
                    db, method, path, status_code, client_ip, error_detail, now_iso
                )

            if status_code < 400:
                await self._track_audit(
                    db,
                    method,
                    path,
                    user_id,
    ...
    async def _track_error(
        self, db, method, path, status_code, client_ip, error_detail, now_iso
    ):
        ...
        if existing:
            ...
            error_type = (
                "parsing_error"
                if "/upload/" in path
                else ("validation_error" if status_code == 422 else "api_error")
            )
