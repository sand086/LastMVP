...
def create_system_router(db, get_current_user):
    """Factory: creates system router with injected dependencies."""
    ...
    async def export_audit_logs(
        ...
        writer.writerow(["Timestamp", "Usuario", "Rol", "Acción", "Entidad", "ID Entidad", "IP", "Status"])
        for l in logs:
        ...
        for u in all_users:
            ...
            if not recent:
                issues.append({
                    ...
                    "description": f"'{u.get('name','')}' ({u.get('email','')}) sin actividad en 30+ días",
    ...
    async def get_integrity_results(user: dict = Depends(get_current_user)):
        ...
        if not result:
            return {"message": "No se ha ejecutado ninguna validación", "issues": [], "total_issues": 0}
        return result
    ...
    async def export_integrity_report(user: dict = Depends(get_current_user)):
        ...
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(["Tipo", "Severidad", "Entidad", "ID", "Descripción", "Acción sugerida"])
