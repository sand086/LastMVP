    ...
    "delivery_types": {
        "exitosa": {
            ...
            "required_evidence": [
                {
                    "key": "foto_fachada",
                    "label": "Foto de fachada del domicilio",
                    "required": True,
                    "weight": 30,
                },
                {
                    "key": "foto_paquete_guia",
                    "label": "Foto del paquete con guía legible",
                    "required": True,
                    "weight": 35,
                },
                {
                    "key": "foto_receptor",
                    "label": "Foto del receptor",
                    "required": True,
                    "weight": 35,
                },
            ],
        ...
        "terceros": {
            ...
            "required_evidence": [
                ...
                {
                    "key": "foto_receptor",
                    "label": "Foto de la persona (tercero)",
                    "required": True,
                    "weight": 25,
                },
                {
                    "key": "nota_driver",
                    "label": "Nota confirmando a quién se entregó",
                    "required": True,
                    "weight": 25,
                },
            ],
            "scoring_rules": {
                "complete": {
                    ...
                    "requires_note": True,
                    "score": 100,
                    "label": "3+ fotos + nota = 100%",
                },
                "photos_no_note": {
                    "min_photos": 3,
                    "requires_note": False,
                    "score": 70,
                    "label": "3+ fotos sin nota = 70%",
                },
                "partial": {
                    "min_photos": 1,
                    "requires_note": False,
        ...
        "fallida": {
            ...
            "required_evidence": [
                {
                    "key": "foto_fachada",
                    "label": "Foto de fachada antes de retirarse",
                    "required": True,
                    "weight": 100,
                },
            ],
...
async def update_quality_criteria(
    data: dict,
    user: dict = Depends(require_role(["coordinator", "developer", "admin"])),
):
    current = await db.system_config.find_one({"key": "quality_criteria"}, {"_id": 0})
    current_value = (
        current.get("value", DEFAULT_CRITERIA) if current else DEFAULT_CRITERIA.copy()
    )
        ...
        for dtype, dconfig in data["delivery_types"].items():
            if dtype in current_value["delivery_types"]:
                if "required_evidence" in dconfig:
                    current_value["delivery_types"][dtype]["required_evidence"] = (
                        dconfig["required_evidence"]
                    )
                if "scoring_rules" in dconfig:
                    current_value["delivery_types"][dtype]["scoring_rules"] = dconfig[
                        "scoring_rules"
                    ]
                if "notes" in dconfig:
    ...
    if "ai_evaluation" in data:
        # ASICOM  VULN-05: RBAC Estricto. Desacoplamiento seguro de Prompts.
        if user.get("role") not in ["developer", "admin"]:
            logger.warning(
                f"[VULN-05] Intento bloqueado de edición de IA por rol: {user.get('role')}"
            )
            raise HTTPException(
                status_code=403,
                detail="Privilegios insuficientes. Solo administradores pueden modificar el motor y prompts de la IA.",
            )
...
async def reset_quality_criteria(
    ...
    return {
        "message": "Criterios restablecidos a valores predeterminados",
        "criteria": default_with_meta,
    }
...
DEFAULT_KPI_TARGETS = {
    ...
    "critical_failure_cap": {
        "trigger_key": "foto_paquete_guia",
        "max_score_if_failed": 40,
    },
}
...
DEFAULT_SLA_BRACKETS = [
    {
        "id": "bracket_1",
        "label": "Mes 1-2",
        "description": "Arranque operativo",
        "target": 65,
        "status": "exceeded",
    },
    {
...
DEFAULT_PENALTY_RULES = [
    {
        "id": "p1",
        "label": "Sin evidencias (0 fotos)",
        "discount_pct": 100,
        "applies_to": "provider",
    },
    {
        "id": "p2",
        "label": "Evidencia parcial (1-2 fotos)",
        "discount_pct": 50,
        "applies_to": "provider",
    },
    {
        "id": "p3",
        "label": "Guía no legible en foto",
        "discount_pct": 60,
        "applies_to": "driver",
    },
    {
        "id": "p4",
        "label": "Sin nota en entrega a terceros",
        "discount_pct": 30,
        "applies_to": "driver",
    },
]
...
DEFAULT_ERROR_CATALOG = [
    {
        "id": "falta_fachada",
        "key": "falta_fachada",
        "label": "Falta foto de fachada del domicilio",
        "applies_to": ["exitosa", "terceros", "fallida"],
        "active": True,
    },
    {
        "id": "guia_no_visible",
        "key": "guia_no_visible",
        "label": "Guía de envío no visible o ilegible",
        "applies_to": ["exitosa", "terceros", "fallida"],
        "active": True,
    },
    {
        "id": "foto_borrosa",
        "key": "foto_borrosa",
        "label": "Foto borrosa o sin foco",
        "applies_to": ["exitosa", "terceros", "fallida"],
        "active": True,
    },
    {
        "id": "sin_foto_receptor",
        "key": "sin_foto_receptor",
        "label": "Falta foto del receptor o tercero",
        "applies_to": ["exitosa", "terceros"],
        "active": True,
    },
    {
        "id": "paquete_fuera_frame",
        "key": "paquete_fuera_frame",
        "label": "Paquete fuera de encuadre o cortado",
        "applies_to": ["exitosa", "terceros", "fallida"],
        "active": True,
    },
    {
        "id": "falta_whatsapp",
        "key": "falta_whatsapp",
        "label": "Falta captura de WhatsApp o SMS de notificación",
        "applies_to": ["fallida"],
        "active": True,
    },
]
...
async def patch_quality_settings(
    ...
    if not section or section not in QUALITY_CONFIG_KEYS:
        raise HTTPException(
            status_code=400,
            detail=f"Section inválida: {section}. Opciones: {QUALITY_CONFIG_KEYS}",
        )
    if value is None:
        raise HTTPException(status_code=400, detail="value requerido")

    #  VULN-05: RBAC Estricto. Protege la configuración de Prompts IA en la BD.
    if section == "ia_config":
        if user.get("role") not in ["developer", "admin"]:
            logger.warning(
                f"[VULN-05] Intento bloqueado de edición de Prompts IA por el usuario: {user.get('email')}"
            )
            raise HTTPException(
                status_code=403,
                detail="Privilegios insuficientes. Solo administradores pueden alterar las instrucciones base de la IA.",
            )
    ...
    await db.system_config.update_one(
        {"key": "quality_criteria"},
        {
            "$inc": {"value.version": 1},
            "$set": {
                "value.updated_by": user.get("name", user["email"]),
