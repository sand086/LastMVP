    ...
    return [
        ...
        # ─────────────── DOCUMENTACIÓN TÉCNICA ───────────────
        {
            ...
            "content": [
                ...
                {"type": "section", "title": "Endpoints principales", "items": ["/api/reports/journeys — datos de rutas con métricas + IDs Routal para cross-reference (routal_route_id, routal_plan_id, routal_plan_label, routal_driver_id, routal_project_id)", "/api/reports/packages — detalle de paquetes (incluye routal_route_id heredado del journey)", "/api/reports/incidents — incidencias registradas", "/api/reports/kpis — KPIs agregados por periodo", "/api/reports/sla — SLA vs targets por proveedor/driver", "/api/reports/attempts — distribución de intentos de entrega", "/api/reports/heatmap — datos geográficos por código postal", "/api/reports/quality — score de calidad de evidencias", "/api/reports/schema — auto-documentación de TODOS los endpoints, fields y guía Power BI ⇄ Routal"]},
        ...
        },
        {
            ...
            "content": [
                ...
                {"type": "section", "title": "Colección: journeys", "items": ["id (uuid str): clave primaria", "client_id, provider_id, driver_id, driver_name", "date (str YYYY-MM-DD): fecha operativa", "status: scheduled / planificada / in_progress / completada / closed / cancelada", "packages_total, packages_delivered, packages_failed", "source: routal / kosmo / null(legacy)", "routal_route_id (sparse, JOIN KEY canónico con Routal route — 1 ruta = 1 journey post-iter79)", "routal_plan_id (sparse, plan padre — agrupa journeys de un mismo plan Routal)", "routal_plan_label, routal_driver_id, routal_project_id (sparse, source=routal)", "routal_synced_at, routal_last_sync_error (worker housekeeping)", "migrated_to_journeys (sparse, journey legacy split en N route-based; oculto en /api/reports/journeys)", "_legacy_incidents_remaining (sparse, contador de orphan incidents)", "start_data: {odometer_start, packages_loaded, started_at, vehicle_condition, fuel_level, photos[]}", "close_data: {closed_at, odometer_end, packages_delivered, packages_failed, packages_to_retry, km_traveled, delivery_rate, notes}", "incidents[]: {id, type, severity, imputability, status, comments}", "last_kosmo_sync_at, next_sync_at (ISO timestamps)"]},
                ...
                {"type": "section", "title": "Endpoint para Power BI", "items": ["GET /api/reports/journeys?date_from=&date_to= devuelve dataset plano por journey con métricas + IDs Routal", "Campos clave: journey_id, packages_delivered, packages_total, delivery_rate, km_traveled, closed_at, provider_name, client_name, driver_name, status", "Cross-reference Routal: routal_route_id (JOIN KEY 1:1 con Routal route), routal_plan_id (plan padre), routal_plan_label, routal_driver_id, routal_project_id", "Para journeys NO Routal (Kosmo/manual) los campos routal_* serán null — filtrar antes del JOIN", "Auth: Authorization: Bearer <refresh_token_canjeado>"]},
                {"type": "tip", "text": "Para Power BI cross-reference con Routal: usa routal_route_id como llave 1:1 entre journeys de LastMile y rutas en Routal. routal_plan_id permite agrupar al nivel plan. Consulta /api/reports/schema → integrations.routal_cross_reference para la receta completa."},
